...
void NodeManager::ProcessDisconnectClientMessage(
  ...
  if (is_worker) {
    ...
    // The client is a driver.
    RAY_CHECK_OK(gcs_client_->driver_table().AppendDriverData(DriverID(client->GetClientId()),
                                                              /*is_dead=*/true));
...
void NodeManager::ProcessPushErrorRequestMessage(const uint8_t *message_data) {
  ...
  RAY_CHECK_OK(gcs_client_->error_table().PushErrorToDriver(driver_id, type, error_message,
                                                            timestamp));
}
...
void NodeManager::ProcessNotifyActorResumedFromCheckpoint(const uint8_t *message_data) {
  ...
  ActorID actor_id = from_flatbuf<ActorID>(*message->actor_id());
  ActorCheckpointID checkpoint_id = from_flatbuf<ActorCheckpointID>(*message->checkpoint_id());
